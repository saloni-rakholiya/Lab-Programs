"""
Usage: 
python <filename> <srcNode> <destNode> <protocol> <interval - optional, default is 1.0>  
Example: 
python simulate.py 1 7 tcp 10.0
"""
...
def main():
    [s1,d1,protocol] = sys.argv[1:4]
    interval = 1.0
    if(len(sys.argv)>=5):
        interval = float(sys.argv[4])
    ns = Simulator(interval)
    ns.throughput(protocol, int(d1))
    ns.bandwithUtilisation(protocol, int(d1))
    ns.end_to_end_delay([int(s1)], [int(d1)])
    ns.pdr([int(s1)], [int(d1)])
